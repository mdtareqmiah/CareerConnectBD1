...
<div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <a href="{{ route('admin.communications.index', ['tab' => 'tickets']) }}" class="btn btn-sm btn-outline-secondary">Back</a>
</div>

<div class="card border-0 shadow-sm mb-3">
    <div class="card-body d-flex flex-wrap gap-2 align-items-center">
        <span class="badge bg-secondary">User: {{ $ticket->user?->name }}</span>
        ...
        <form method="POST" action="{{ route('admin.support-tickets.status', $ticket) }}" class="d-flex gap-2 ms-auto">
            @csrf
            @method('PATCH')
...
<div class="card border-0 shadow-sm mb-3" style="max-height: 520px; overflow-y: auto;" id="adminTicketThread">
    <div class="card-body">
...
<div class="card border-0 shadow-sm">
    <div class="card-body">
        <form method="POST" action="{{ route('admin.support-tickets.replies.store', $ticket) }}" enctype="multipart/form-data">
            @csrf
            <div class="mb-3">
        ...
        <form method="POST" action="{{ route('admin.support-tickets.destroy', $ticket) }}" class="mt-2 text-end">
            @csrf
            @method('DELETE')
            <button class="btn btn-outline-danger" onclick="return confirm('Delete this ticket?')">Delete Ticket</button>
        </form>
    </div>
</div>
@endif
